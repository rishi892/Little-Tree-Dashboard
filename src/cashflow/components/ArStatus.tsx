...
import { fetchArStatus, type ArStatusResult } from '../api';
...
export function ArStatus() {
 const [data, setData] = useState<ArStatusResult | null>(null);
 ...
 async function load() {
   setLoading(true);
   setError(null);
   try { setData(await fetchArStatus()); }
 ...
 if (loading && !data) return (
   <div className="page-head"><h1 className="page-title">AR Status · {new Date().getUTCFullYear()}</h1><div className="page-sub">Loading from LT Financials…</div></div>
 );
 if (error) return (<>
   <div className="page-head"><h1 className="page-title">AR Status</h1></div>
 ...
 return (
   <>
     <div className="page-head">
       <div>
         <h1 className="page-title">AR Status · {data.year}</h1>
         <div className="page-sub">
           Cash collection bucketed by <strong>paid date</strong> (Dec invoice paid in {data.year} counts as {data.year}). Gelato excluded - it has its own AR pipeline at Net 97. As of {data.asOfDate}.
         </div>
       </div>
     ...
     {/* Data-quality drilldown: paid amounts on the sheet that have no paidDate.
         Collapsed by default so it doesn't dominate the page; expand to see
         which invoices need a date filled in. */}
     {data.paidWithMissingDate > 0 && (
       <CollapsibleSection
         ...
         sub="These dollars have a paid amount but no paid-date column - excluded from the monthly/weekly buckets above. Expand to see which rows to fix."
       >
     ...
     <div className="section">
       <div className="section-head">
         <div>
           ...
           <div className="section-sub">Mon-Sun weeks. Paid invoices grouped by paid date.</div>
         </div>
       </div>
       <div className="table-wrap">
     ...
     <div className="section">
       <div className="section-head">
         <div>
           ...
           <div className="section-sub">$ that landed in each calendar month. Reflects when cash actually arrived, not invoice issue.</div>
         </div>
       </div>
       <div className="table-wrap">
         <table className="data-table">
     ...
     <div className="section">
       <div className="section-head">
         <div>
           ...
           <div className="section-sub">Days since invoice issue. 91+ is the collection-at-risk pile.</div>
         </div>
       </div>
